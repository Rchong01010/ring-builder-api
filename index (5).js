...
// Fetch product by full SKU (this returns images)
async function fetchBySku(sku) {
  try {
    ...
    return response.Products && response.Products[0] ? response.Products[0] : null;
  } catch (error) {
    console.log(`Error fetching SKU ${sku}: ${error.message}`);
    return null;
  }
}

// Fetch one series from Stuller using SeriesId (to get a valid SKU)
async function fetchSeriesForSku(seriesId) {
  try {
    const response = await makeStullerRequest({
      "Include": ["All"],
      "SeriesId": seriesId,
...
function transformProduct(product, forcedStyle = null) {
  ...
  return {
    ...
    sku: product.SKU,
    series: series,
    name: product.GroupDescription || product.Description || `Series ${series}`,
...
// Refresh the product cache - two-step: SeriesId -> SKU -> full product with images
async function refreshCache() {
  ...
  for (const [key, styleName] of Object.entries(styleMap)) {
    ...
    for (const seriesId of seriesList) {
      ...
      // Step 1: Get a SKU from this series
      const seriesProducts = await fetchSeriesForSku(seriesId);
      
      if (seriesProducts.length > 0) {
        const firstSku = seriesProducts[0].SKU;
        
        // Step 2: Fetch full product with images using SKU
        const fullProduct = await fetchBySku(firstSku);
        
        if (fullProduct) {
          const transformed = transformProduct(fullProduct, styleName);
          
          if (transformed) {
            seenSeries.add(seriesId);
            allProducts.push(transformed);
            const hasImg = transformed.image ? '✓' : '✗';
            console.log(`  ${hasImg} ${seriesId}: ${transformed.name}`);
          }
        } else {
          console.log(`  ✗ ${seriesId}: Could not fetch full product`);
        }
      } else {
