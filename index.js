...
// Sample SKUs for each series (one known working SKU per series to fetch with images)
// We'll build these dynamically
...
function buildStullerImageUrl(seriesId) {
  // Stuller's standard image URL pattern
  return `https://stuller.scene7.com/is/image/qudera/${seriesId}_qudera?$qudera_productfamilythumbnail$`;
}
...
function transformProduct(product, forcedStyle = null) {
  ...
  // Fallback: build URL from series number
  if (!primaryImage && series) {
    primaryImage = buildStullerImageUrl(series);
  }
  ...
  for (const [key, styleName] of Object.entries(styleMap)) {
    ...
    for (const seriesId of seriesList) {
      ...
      if (products.length > 0) {
        // Find product with image, or use first one
        ...
        if (transformed) {
          ...
          const hasImg = (productWithImage.Images && productWithImage.Images.length > 0) ? '✓' : '~';
